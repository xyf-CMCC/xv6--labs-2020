...
/*
 * create a direct-map page table for the kernel.
内核进程需要依赖内核页表内一些固定的映射才能正常工作，例如UART控制、硬盘界面、中断控
制等。在kernel/vm.c中，kvminit函数原本只为全局内核页表kernel_pagetable添加这些
映射，所以接下来大幅度改动kernel/vm.c，使其他进程也可以创建独享的内核页表。
...
// kernel/vm.c实现kama_kvminit_newpgtbl函数，创建一个页表并初始化映射，返回这个页表：
pagetable_t
